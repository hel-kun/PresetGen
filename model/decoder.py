...
# 作ったけど使わない遺産1(設計をミスっている)
class DualStreamTransformerDecoder(nn.Module):
    def __init__(
        self,
        decoder_layer: DualStreamTransformerDecoderLayer,
        num_layers: int,
    ...
    def forward(
        ...
        cross_stream_mask: Optional[Tensor] = None,
        tgt_key_padding_mask: Optional[Tensor] = None,
        memory_key_padding_mask: Optional[Tensor] = None,
        cross_stream_key_padding_mask: Optional[Tensor] = None,
        ...
        for mod in self.layers:
            output = mod(
                ...
                cross_stream_mask=cross_stream_mask,
                tgt_key_padding_mask=tgt_key_padding_mask,
                memory_key_padding_mask=memory_key_padding_mask,
                cross_stream_key_padding_mask=cross_stream_key_padding_mask,
    ...
    def _get_seq_len(self, src: Tensor, batch_first: bool) -> int:
        if src.is_nested:
            return None
        else:
            src_size = src.size()
...
# 作ったけど使わない遺産2  
class CategoricalParamDecoder(nn.Module):
    def __init__(
        self,
        embed_dim: int = 512,
        num_heads: int = 8,
        ...
        if categorical_param_size is None:
            logging.warning("categorical_param_sizeが指定されていません。デフォルト値を使用します。")
            self.categorical_param_sizes = {name: 5 for name in CATEGORICAL_PARAM_NAMES}
        else:
            self.categorical_param_sizes = categorical_param_size
        ...
        self.decoder = DualStreamTransformerDecoder(
            decoder_layer,
        ...
        self.output_heads = nn.ModuleDict({
            name: nn.Linear(embed_dim, size) for name, size in self.categorical_param_sizes.items()
        })
    
    def forward(self, tgt, memory, cross_stream_memory, **kwargs):
        x = self.decoder(
            tgt,
            memory,
            cross_stream_memory,
        ...
        for name, head in self.output_heads.items():
            outputs[name] = head(x)  # (batch, seq_len, param_size)
        
        return outputs

# 作ったけど使わない遺産3
class ContinuiusParamDecoder(nn.Module):
    def __init__(
        self,
        embed_dim: int = 512,
        num_heads: int = 8,
    ...
    def forward(self, tgt, memory, cross_stream_memory, **kwargs):
        ...
        outputs = {}
        for name, head in self.output_head.items():
            outputs[name] = head(x).squeeze(-1)  # (batch, seq_len)
        
        return outputs

# TODO: PresetGenDecoderの実装
